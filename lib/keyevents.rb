...
module Operations
  ...
  def keyevent(args)
    default_device.keyevent(args)
  end

  def text(args)
    default_device.text(args)
  end

  def run(path)
    default_device.run(path)
  end
  def run_adb(args, adb_opts = {})
    default_device.run_adb(args, adb_opts = {})
  end

  def run_adb_shell(args, adb_args = {})
    default_device.run_adb_shell(args, adb_args = {})
  end
  ...
  class Device
    ...
    def text(args)
      cmd = "#{adb_command} -d shell input text #{args}"
      log "Entering text"
      raise "Could enter a text" unless system(cmd)
    end

    def keyevent(args)
      cmd = "#{adb_command} -d shell input keyevent #{args}"
      log "Taking a picture"
      raise "Could not take a picture" unless system(cmd)
    end
...
# ADB commands definitions added - adb shell
  def run_adb(args, adb_opts = {}, &block)
      adb_arg = ""
    ...
    def run_adb_shell(args, adb_args = {}, &block)
      args = "shell #{args}"
